            ...
            <table class="w-full table-fixed">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        @if($statusDelete == $post->id)
                        <button wire:click="delete({{ $post->id }})" class="px-4 py-2 font-bold text-white bg-red-500 rounded hover:bg-red-700">Yakin?</button>
                        <button wire:click="cancelDelete()" class="px-4 py-2 font-bold text-white rounded bg-cool-gray-400 hover:bg-cool-gray-600">Batal</button>
                        @else
                        <button wire:click="edit({{ $post->id }})" class="px-4 py-2 font-bold text-white bg-blue-500 rounded hover:bg-blue-700">Edit</button>
                            <button wire:click="statusDelete({{ $post->id }})" class="px-4 py-2 font-bold text-white bg-red-500 rounded hover:bg-red-700">Delete</button>
                        @endif
                        </td>
                    </tr>
                    @endforeach
